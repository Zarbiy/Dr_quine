section .data
    ...
    msg db "section .data%1$c    base_name db %2$cSully_%%d.s%2$c,0%1$c    file_name db 15 dup(0)%1$c    msg db %2$c%3$s%2$c,0%1$c    base_exec db %2$cnasm -f elf64 Sully_%%1$d.s -o Sully_%%1$d.o && gcc -o Sully_%%1$d Sully_%%1$d.o -nostartfiles && ./Sully_%%1$d%2$c,0%1$c    exec_name db 70 dup(0)%1$c%1$csection .text%1$c    extern printf, sprintf, dprintf, exit, system, open, close%1$c    global _start%1$c%1$c_start:%1$c    mov r12, %4$d%1$c    cmp r12, 0%1$c    jle end%1$c%1$c    dec r12%1$c%1$c    lea rdi, [rel file_name]%1$c    lea rsi, [rel base_name]%1$c    mov edx, r12d%1$c    xor rax, rax%1$c    mov rax, [rel sprintf wrt ..got]%1$c    call rax%1$c%1$c    mov rdi, file_name%1$c    mov rsi, 577%1$c    mov rdx, 0o777%1$c    xor rax, rax%1$c    mov rax, [rel open wrt ..got]%1$c    call rax%1$c    cmp rax, 0%1$c    jl .open_error%1$c%1$c    mov rdi, rax%1$c    lea rsi, [rel msg]%1$c    mov rdx, 10%1$c    mov rcx, 34%1$c    lea r8, [rel msg]%1$c    mov r9, r12%1$c    xor rax, rax%1$c    mov rax, [rel dprintf wrt ..got]%1$c    call rax%1$c%1$c    xor rax, rax%1$c    mov rax, [rel close wrt ..got]%1$c    call rax%1$c%1$c    lea rdi, [rel exec_name]%1$c    lea rsi, [rel base_exec]%1$c    mov rdx, r12%1$c    xor rax, rax%1$c    mov rax, [rel sprintf wrt ..got]%1$c    call rax%1$c%1$c    lea rdi, [rel exec_name]%1$c    xor rax, rax%1$c    mov rax, [rel system wrt ..got]%1$c    call rax%1$c%1$c    mov rdi, 0%1$c    mov rax, [rel exit wrt ..got]%1$c    call rax%1$c%1$c.open_error:%1$c    mov rdi, 1%1$c    mov rax, [rel exit wrt ..got]%1$c    call rax%1$c%1$cend:%1$c    mov rdi, 0%1$c    mov rax, [rel exit wrt ..got]%1$c    call rax",0
    base_exec db "nasm -f elf64 Sully_%1$d.s -o Sully_%1$d.o && gcc -o Sully_%1$d Sully_%1$d.o -nostartfiles && ./Sully_%1$d",0
